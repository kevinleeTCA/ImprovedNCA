...
//测试对于任意一个随机的ISD对应的backward KSD characteristic中固定位置的个数 using Sampling resistance
/*参数：
d:输入差分的最大汉明重量
...
void BW_ave_KSD_pos_with_fixed_diff_with_SP(u32 d,u32 num_ISD,u32 N){
	...
	for(int D=1;D<=num_ISD;D++){
		...
		for(int j=0;j<d;j++){
			pos[j]=rc4() % 160;
			while(isExistWithSP_BW(pos,j,pos[j],12))		//保证随机位置不会出现重复
				pos[j]=rc4() % 160;
		}
		//初始化 输入差分为全0
		u8 ISD[LEN];
		...
		for(int i=0;i<N;i++){
			...
			//将状态代入grain中,获得对应的长度为KSLen的backward密钥流，并输出其差分
